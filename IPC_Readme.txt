...
i)If sender sends the message before the reciver is ready for reciving.
...
ii)If receiver attempts to recive a message, before a sender has requested
...
2. PIPES IPC mechanism
->The term Pipe is used to connect a data flow from one process to another.
->The output of one process is piped(attached) to the input on another process.

3. SIGNALS
->A signal is an event generated by a UNIX/LINUX system in response to some condition, 
  upon receipt of which a process may in turn take some action.
->"raise" is the term used to indicate the generation of a signal.
->"catch" is the term used to indicate reception of the signal.
->Signals are raised due to some error conditions like
  i)memory segmentation violations
  ii)floating point processor errors
  iii)illegal instructions
->They are generated by shell handlers to cause interrupts and can also be explicitly sent
  from one process to another, as a way of passing information or modifying behavior.

  SIGNAL NAME          DESCRIPTION
  SIGABORT             Process abort
  SIGALARM             Alarm clock
  SIGFPE               Floating point expression
  SIGHUP               Hangup
  SIGILL               Illegal instruction
  SIGINT               Termianl interrupt                  ---->ctrl+c
  SIGKILL              Kill
  ...
  SIGCGILD             Child process has stopped or exited
  ...
  SIG_DFL              Restore default behaviour
